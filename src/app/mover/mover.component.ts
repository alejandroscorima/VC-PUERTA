...
export class MoverComponent implements OnInit {
  ...
  dataSourceItems: MatTableDataSource<Cliente>;
  ...
  ngOnInit() {
    ...
    this.clientesService.getClients().subscribe((cList:Cliente[])=>{
      this.clients=cList;
      console.log(this.clients);
      this.dataSourceItems = new MatTableDataSource(this.clients);
      this.dataSourceItems.paginator = this.paginator.toArray()[0];
      this.dataSourceItems.sort = this.sort.toArray()[0];
    });
    this.tipos=['MAQUINAS','OTROS'];
  }
...
/*     var dialogRef;
    ...
    dialogRef=this.dialog.open(DialogDatos,{
      data:this.items[index],
    }) */

    var index = this.items.findIndex(m=>m.codigo==code);

    this.items[index].cantidad=2;
  }
